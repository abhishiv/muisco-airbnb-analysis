...
(function() {
  ...
  function initiateSpatialNavigation() {
    ...
    window.addEventListener("keydown", function(e) {
      ...
      if (e.keyCode === TAB_KEY_CODE) startingPosition = null;
  ...
  function navigate(dir) {
    ...
    if (
      ...
      // 5-2
      if (focusingController(eventTarget.spatialNavigationSearch(dir), dir))
        return;
    ...
    let parentContainer = container.parentElement
      ? container.parentElement.getSpatialNavigationContainer()
      : null;
    ...
    while (parentContainer) {
      if (
        focusingController(
          eventTarget.spatialNavigationSearch(
            dir,
            container.focusableAreas(),
            container
          ),
          dir
        )
...
//            }
  ...
  function spatNavCandidates(element, dir, candidates, container) {
    ...
    if (!candidates || candidates.length <= 0) {
      if (
        (isContainer(targetElement) || targetElement.nodeName === "BODY") &&
  ...
  function createSpatNavEvents(option, element, elm, direction) {
    ...
    switch (option) {
      case "beforefocus":
        triggeredEvent = new CustomEvent("navbeforefocus", {
          bubbles: true,
          cancelable: true,
          detail: data
        });
      ...
      case "beforescroll":
        triggeredEvent = new CustomEvent("navbeforescroll", {
          bubbles: true,
          cancelable: true,
          detail: data
        });
      ...
      case "notarget":
        triggeredEvent = new CustomEvent("navnotarget", {
          bubbles: true,
          cancelable: true,
          detail: data
        });
  ...
  function findStartingPoint() {
    ...
    if (
      ...
      (startingPoint === document.body &&
        !document.querySelector(":focus")) /* body isn't actually focused*/
    ) {
  ...
  function isBeingRendered(element) {
    ...
    return (
      ...
      !(
        (element.style.width === "0px" || element.style.width === 0) &&
  ...
  function isVisible(element) {
    ...
    return (
      !element.parentElement ||
      (isVisibleStyleProperty(elementStyle) && hitTest(element))
    );
  }
  ...
  function hitTest(element) {
    ...
    const middleElem = document.elementFromPoint(
    ...
    const leftTopElem = document.elementFromPoint(
    ...
    const leftBottomElem = document.elementFromPoint(
      elementRect.left + offsetX,
    ...
    const rightTopElem = document.elementFromPoint(
    ...
    const rightBottomElem = document.elementFromPoint(
      elementRect.right - offsetX,
      elementRect.bottom - offsetY
    );
  ...
  function getDistance(rect1, rect2, dir) {
    ...
    switch (dir) {
      ...
      case "right":
        ...
        if (!isAligned(rect1, rect2, dir)) orthogonalBias = rect1.height / 2;
      ...
      case "down":
        B = P2;
        // If not aligned => add bias
        if (!isAligned(rect1, rect2, dir)) orthogonalBias = rect1.width / 2;
    ...
    const D = intersectionRect
      ? Math.sqrt(intersectionRect.width * intersectionRect.height)
      : 0;
  ...
  function getEntryAndExitPoints(dir = "down", rect1, rect2) {
    ...
    switch (dir) {
      case "left":
        ...
        points.entryPoint[0] =
          rect2.right < rect1.left ? rect2.right : rect1.left;
        break;
      case "up":
        points.exitPoint[1] = rect1.top;
        points.entryPoint[1] =
          rect2.bottom < rect1.top ? rect2.bottom : rect1.top;
        break;
      case "right":
        points.exitPoint[0] = rect1.right;
        points.entryPoint[0] =
          rect2.left > rect1.right ? rect2.left : rect1.right;
        break;
      case "down":
        points.exitPoint[1] = rect1.bottom;
        points.entryPoint[1] =
          rect2.top > rect1.bottom ? rect2.top : rect1.bottom;
        break;
    }
  ...
  function getIntersectionRect(rect1, rect2) {
    ...
    if (
      !(newLocation[0] >= newMaxPoint[0] || newLocation[1] >= newMaxPoint[1])
    ) {
      // intersecting-cases
      return {
      ...
      if (
        ...
        if (Array.isArray(candidates) && candidates.length > 0) {
          return findCandidate
            ? spatNavCandidates(eventTarget, dir, candidates)
...
//      }
      ...
      while (parentContainer) {
        const candidates = filteredCandidates(
          eventTarget,
          container.focusableAreas(option),
          dir,
          container
        );

        if (Array.isArray(candidates) && candidates.length > 0) {
          bestNextTarget = eventTarget.spatialNavigationSearch(
            dir,
            candidates,
            container
          );
